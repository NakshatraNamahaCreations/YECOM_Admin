...
function TryToBooking() {
  ...
  const fetchData = async () => {
    try {
      ...
      const falsePayment = paymentRes.allUsers.filter(
        (item) => item.paymentStatus === false
      );
      setAllPayments(falsePayment);

      // const userRes = await getData(apiUrl.GET_ALL_USER);
      // setAllUsers(userRes.data);

      // const handlingPaymentData = paymentRes.allUsers?.map((ele) => {
      //   const user = userRes.data?.find((item) => item._id === ele.userId);
      //   return {
      //     ...ele,
  ...
  return (
    <div className="mt-3">
      <div className="d-flex justify-content-between mt-3">
        ...
        <div className="">
          <button style={styles.createCourseBtn} onClick={handleShowFilter}>
            Filter
          </button>
        </div>
      </div>
